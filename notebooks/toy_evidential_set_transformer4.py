...
class HParams:
    ...
    grid_w: int = 100
    n_pixels: int = 25 * 100
    warmup_fixed: int = 32             # first 32 measurements use a fixed pattern
...
def _load_field_source_idx(npz_path: str):
    """
    優先順: <same_dir>/field_source_idx.npy -> NPZ 埋め込みキー -> meta から推定
    戻り値は shape (n_fields,) の int64 ベクトル（各フィールドのグローバルID）
    """
    ...
    try:
        with np.load(npz_path, allow_pickle=True) as d:
            ...
            # (3) meta から推定: 連番 [start_index, start_index + n_fields)
            if "meta" in d.files:
                try:
                    meta = json.loads(d["meta"][0])
                    if ("start_index" in meta) and ("n_fields" in meta):
                        start = int(meta["start_index"])
                        nF    = int(meta["n_fields"])
                        return np.arange(start, start + nF, dtype=np.int64)
                except Exception:
                    pass
    except Exception:
        pass
...
class ERTOracleDataset(Dataset):
    ...
    def __init__(
        self,
        ert_npz_path: str,                 # ← 33回目以降(学習対象)
        oracle_npz_path: str,
        hp: HParams = HP,
        pick_scale_index: int = 0,
        max_fields: int | None = None,
        field_offset: int = 0,
        warmup_npz_path: str | None = None,    # ← 追加: 1〜32回目(履歴のみ)
        warmup_pick: str = "first32"           # "first32" / "stride"
    ):
        super().__init__()
        self.hp = hp

        # 33回目以降に使うアクティブ設計の ERT（従来どおり）
        ...
        # === ここから 追加：field_source_idx の一致検証 ===
        # アクティブ側
        fidx_act_full = _load_field_source_idx(ert_npz_path)
        if fidx_act_full is None:
            raise FileNotFoundError(
                f"[field_source_idx] not found for active NPZ: {ert_npz_path}\n"
                f"  Expected: <same_dir>/field_source_idx.npy or embedded key in NPZ."
            )
        fidx_act = np.asarray(fidx_act_full, dtype=np.int64)

        # ウォームアップ側（ある場合のみ）
        fidx_warm = None
        if warmup_npz_path is not None:
            fidx_warm_full = _load_field_source_idx(warmup_npz_path)
            if fidx_warm_full is None:
                raise FileNotFoundError(
                    f"[field_source_idx] not found for warmup NPZ: {warmup_npz_path}\n"
                    f"  Expected: <same_dir>/field_source_idx.npy or embedded key in NPZ."
                )
            fidx_warm = np.asarray(fidx_warm_full, dtype=np.int64)

        # 長さ検証（任意だが早期に気づけるように）
        if fidx_warm is not None and fidx_warm.shape[0] != fidx_act.shape[0]:
            raise ValueError(
                "[field_source_idx] length mismatch between warmup and active.\n"
                f"  warmup: {fidx_warm.shape[0]}  vs  active: {fidx_act.shape[0]}"
            )

        # スライスして比較（今回使うフィールド範囲のみ）
        ...
        if fidx_warm is not None:
            ...
            if act_slice.shape != warm_slice.shape or not np.array_equal(act_slice, warm_slice):
                # どこが違うかも出す
                if act_slice.shape == warm_slice.shape:
                    diff_mask = act_slice != warm_slice
                    first_k = np.flatnonzero(diff_mask)[:10]  # 多すぎる時のために先頭10件
                    msg_diff = ", ".join(
                        f"i={start_f+int(i)}: warm={int(warm_slice[i])}, act={int(act_slice[i])}"
                        for i in first_k
                    )
                else:
                    msg_diff = f"(shape) warm={warm_slice.shape}, act={act_slice.shape}"

                raise ValueError(
                    "[field_source_idx] mismatch between warmup and active for the selected field range.\n"
                    f"  range: [{start_f}:{end_f})  use_Nf={use_Nf}\n"
                    f"  examples: {msg_diff}"
                )
        # === 一致検証ここまで ===
        ...
        for i in range(use_Nf):
            ...
            if (Hc, Wc) != (gh, gw):
                ...
                mo_resized = mo
            y_bin = mo_resized.astype(np.float32).reshape(-1)

            sets_list, masks_list = [], []

            # ----- (1) ウォームアップ 32 ステップ：Wenner の 32 件を履歴として使用 -----
            if Dn_all_warm is not None:
                ...
                for t in idx:
                    ...
                    masks_list.append(torch.ones((1, 5), dtype=torch.bool))  # 使うけど損失には寄与しない（下で start=32）
            else:
                # 従来のダミー（互換）
            ...
            # ----- (2) アクティブ 120 ステップ：学習対象（従来どおり） -----
            row0 = i * R_act
            take = min(hp.active_len, R_act)
            Dn_i = Dn_all_act[row0: row0 + take]
            yi   = y_all_act[row0: row0 + take]
            if take < hp.active_len:
                pad = hp.active_len - take
                Dn_i = np.concatenate([Dn_i, np.tile(Dn_i[-1:], (pad, 1))], axis=0)
                yi   = np.concatenate([yi,   np.tile(yi[-1:],   (pad,   ))], axis=0)
            ...
            self.samples.append((sets_list, masks_list, torch.from_numpy(y_bin)))
    def __len__(self):
...
def run_train(epochs=5, n_train=16, n_val=4, save_dir="./outputs"):
    ...
    # 使うファイル
    ERT_NPZ   = "../data/interim/surrogate_ds/ert_surrogate.npz"
    ORACLE_NPZ= "../data/processed/oracle_pairs/latent_oracle_pairs_aligned.npz"

    # --- DataSets: 学習/検証ともに ERTOracleDataset を使う ---
    train_ds = ERTOracleDataset(
        ert_npz_path="../data/interim/surrogate_ds/ert_surrogate.npz",             # 33回目以降 (学習)
        oracle_npz_path="../data/processed/oracle_pairs/latent_oracle_pairs_aligned.npz",
        hp=HP, pick_scale_index=0,
        max_fields=n_train, field_offset=0,
        warmup_npz_path="../data/interim/ert_wenner_subset/ert_surrogate_wenner.npz",   # 1〜32回目 (履歴)
        warmup_pick="first32"   # or "stride"
    )
    val_ds = ERTOracleDataset(
        ert_npz_path="../data/interim/surrogate_ds/ert_surrogate.npz",
        oracle_npz_path="../data/processed/oracle_pairs/latent_oracle_pairs_aligned.npz",
        hp=HP, pick_scale_index=0,
        max_fields=n_val, field_offset=n_train,
        warmup_npz_path="../data/interim/ert_wenner_subset/ert_surrogate_wenner.npz",
        warmup_pick="first32"
    )
