...
class ToyModel(nn.Module):
    def __init__(self, hp: HParams, roi_idx: np.ndarray | None = None):
        ...
        self.encoder = SetEncoder(
            ...
            heads=hp.enc_heads, layers=hp.enc_layers,
            pma_seeds=hp.pma_seeds, dropout=hp.enc_dropout
        )
    ...
    def forward_time(self, S_t):
        ...
        ctx = H_t.unsqueeze(1) if H_t.dim() == 2 else H_t
        B = S_t.size(0)
...
# Train / Smoke
# ---------------------------
def run_train(epochs=5, n_train=32, n_val=4, save_dir="./outputs"):
    device = HP.device
