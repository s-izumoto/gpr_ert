...
def run_test_single_field(
    ckpt_path: str,
    out_dir: str = "./outputs_test_single",
    field_offset: int = 0,              # 先頭から何番目のフィールドか
    first_output_step: int = 33,
    ...
    test_ds = ERTOracleDataset(
        ...
        oracle_npz_path=ORACLE_NPZ,
        hp=HP, pick_scale_index=0,
        max_fields=1, field_offset=field_offset,     # ← ここが1フィールド指定
        warmup_npz_path=WARMUP_NPZ,
        warmup_pick="first35",
        active_select="random",
        active_seed=20251021
    ...
    # 保存先
    sample_dir = os.path.join(out_dir, f"field_{field_offset:04d}")
    os.makedirs(sample_dir, exist_ok=True)

    # ===== 推論 =====
    for (sets_list, masks_list, _y_bin) in test_dl:
        sets_list  = [s.to(device) for s in sets_list]
        masks_list = [m.to(device) for m in masks_list]
        alpha_T, beta_T = model.forward_sequence(sets_list, masks_list)  # [1, T, N_roi]
        ...
        for t in range(t_first, t_last + 1):
            a = alpha_T[:, t, :].squeeze(0)
            b = beta_T[:, t, :].squeeze(0)
            alpha_cum += a
            beta_cum  += b
            ...
            # フル格子に復元
            full_p = np.zeros((gh * gw,), dtype=np.float32)
            if roi_idx is None:
                full_p[:] = p
            else:
                full_p[roi_idx] = p
            full_p = full_p.reshape(gh, gw)
            stacked_p.append(full_p)

            # --- NPY 保存 ---
            np.save(os.path.join(sample_dir, f"p_step_{t+1:03d}.npy"), full_p)

            # --- PNG 保存（imshow, 軸は非表示に）---
            plt.figure()
            plt.imshow(full_p, origin="upper")
            plt.axis("off")
            plt.tight_layout(pad=0)
            plt.savefig(os.path.join(sample_dir, f"p_step_{t+1:03d}.png"), dpi=150, bbox_inches="tight", pad_inches=0)
            plt.close()

            if save_alpha_beta:
                full_a = np.zeros((gh * gw,), dtype=np.float32)
                full_b = np.zeros((gh * gw,), dtype=np.float32)
                if roi_idx is None:
                    full_a[:] = a.detach().cpu().numpy()
                    full_b[:] = b.detach().cpu().numpy()
                else:
                    full_a[roi_idx] = a.detach().cpu().numpy()
                    full_b[roi_idx] = b.detach().cpu().numpy()
                np.save(os.path.join(sample_dir, f"alpha_step_{t+1:03d}.npy"), full_a.reshape(gh, gw))
                np.save(os.path.join(sample_dir, f"beta_step_{t+1:03d}.npy"),  full_b.reshape(gh, gw))

        # 1ファイルに圧縮（任意）
        stacked_p = np.stack(stacked_p, axis=0)  # [#steps, gh, gw]
        np.savez_compressed(
            os.path.join(sample_dir, f"probs_steps_{t_first+1:03d}_{t_last+1:03d}.npz"),
            steps=np.arange(t_first+1, t_last+1+1, dtype=np.int32),
            p=stacked_p
        )

    print(f"[TEST single] saved to: {sample_dir}")
