...
"""
build/cluster_pca_kmeans.py
---------------------------
Clusters PCA latent vectors (Z) with KMeans / MiniBatchKMeans and optionally
saves centroid/medoid reconstructions using a PCA joblib.

This is a direct "build" split of the original cluster_pca_kmeans.py.
"""
...
def main():
    ap = argparse.ArgumentParser(description="Cluster PCA latent matrix Z (from Z.npz) and save labels/meta/optional images.")
    ...
    # Flags with True-by-default behavior:
    ...
    if args.save_centroids:
        ...
        if args.save_medoids and args.samples_per_cluster > 0:
            ...
            medoid_records = []  # (cluster, order, src_idx, png_relpath)
            ...
            if len(medoid_records) > 0:
                ...
                # 列: [cluster, order, src_idx, png_relpath]
                np.savez_compressed(out_dir / "medoids_index.npz",
