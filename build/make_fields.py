# -*- coding: utf-8 -*-
"""
make_fields.py — Synthetic log10 resistivity (Ω·m) field generator
-------------------------------------------------------------------
Purpose
  Create a labelled dataset of 2‑D log10(resistivity) fields for multiple
  environmental/geologic scenarios ("cases"). The script procedurally
  synthesizes smooth layered backgrounds, adds controlled heterogeneity,
  and (optionally) writes PNG previews. It saves everything into a single
  compressed NPZ file along with compact metadata.

Key outputs (written under --out, default ./ert_ds)
  - dataset.npz
      X_log10  : float32 array, shape (N, NZ, NX), log10(ρ[Ω·m])
      y        : int64 array, shape (N,), class id (see label_map.json)
      cases    : object array, shape (N,), case name per sample
  - label_map.json : mapping from case name -> integer id
  - meta.json      : grid size, counts, global min/max, notes
  - previews/*.png : (optional) a few per class, aspect = grid aspect

Cases generated (see GENS registry)
  TRACER     : shallow tracer-like conductive patches in layered host
  GEOLOGY    : wavy, mostly layered geology with gentle heterogeneity
  SURFACE    : near‑surface gaussian lobes blended into background
  SEAWATER   : coastal wedge / intrusion style deepening conductor
  WATERTABLE : undulating water table separating two facies
  RESISTIVE  : resistive inclusions and/or a shallow resistive cap

Coordinate convention
  The field is indexed as (NZ, NX) = (depth, x). NZ rows from top (0)
  to bottom (NZ-1), NX columns from left (0) to right (NX-1).

Numerical conventions
  - All values are *log10(resistivity in Ω·m)* (log10 ρ). A global clamp
    keeps results in [log10(0.2), log10(1e4)].
  - Smooth transitions are constructed with logistic ramps whose
    thickness varies along x; additional sharpening is applied over
    selected spans to emulate crisp vs diffuse contacts.
  - Lateral/vertical correlation of heterogeneity is controlled by
    gaussian smoothing (σ_z, σ_x) in *cells*.

Reproducibility
  Pass --seed <int> to seed both NumPy and Python's random.

Typical usage
  python make_fields.py --out ./ert_ds --nz 100 --nx 200 --per-class 200 \
                        --save-png --previews-per-class 4 --dpi 140 --seed 42

Loading in Python
  d = np.load('ert_ds/dataset.npz'); X = d['X_log10']; y = d['y']; cases = d['cases']

Notes
  - SciPy is optional. If unavailable, a minimal gaussian_filter fallback
    is used (slower, but fine for moderate sizes).
  - This script *does not* split into train/val/test; keep it pure and
    split later per your experiment setup.
"""
...
# Logistic blend thickness (in depth *cells*) for deep homogenization
DEEP_SMOOTH_THICK: Tuple[float, float] = (8.0, 16.0)

# Optional SciPy smoothing (faster/nicer). Fallback included if SciPy not present.
try:
    from scipy.ndimage import gaussian_filter  # type: ignore
except Exception:
    # Lightweight gaussian_filter fallback (reflect padding; separable 1‑D conv)
...
# μ, σ in dex; lo/hi are log10 clamps; final global clamp is applied later
# -----------------------------
FACIES: Dict[str, Tuple[float, float, float, float]] = {
...
def draw_log10_rho(facies_name: str) -> float:
    """Sample one log10(ρ[Ω·m]) value from the prior for a given facies name."""
...
def _sharpen_gate(A: np.ndarray, gamma: np.ndarray) -> np.ndarray:
    ...
    `gamma` may broadcast over (NZ, NX) or (1, NX).
    """
    eps = 1e-6
...
def make_wavy_interfaces(
    ...
    NZ: int,
    depths: List[float],
    amp_frac: Tuple[float, float] = (0.03, 0.10),
    wavelen_frac: Tuple[float, float] = (0.25, 0.9),
    min_sep: int = 3,
    zmin: int = 1,
    zmax: int | None = None,
) -> np.ndarray:
    """Create per‑column interface depths with sinusoidal waviness.

    Returns
      D : float32 array, shape (L-1, NX). Monotonic per column, clipped to [zmin, zmax].
    """
    if zmax is None:
        zmax = NZ - 2
    ...
    # Clip to shallow window
    D = np.clip(D, max(1, zmin), min(zmax, NZ - 2))

    # Enforce non‑crossing and minimum separation
    for j in range(NX):
        col = np.sort(D[:, j])
...
def smooth_layer_weights(
    ...
    NZ: int,
    t_range=None,
    t_soft_range: Tuple[float, float] = (7.0, 15.0),
    t_crisp_range: Tuple[float, float] = (0.8, 2.2),  # thinner → crisper
    crisp_fraction: float = 0.55,  # average fraction of crisp interface
    gate_gamma_range: Tuple[float, float] = (1.0, 2.0),  # extra sharpening
    t_smooth_x: float = 20.0,
) -> np.ndarray:
    """Convert interfaces to (L) smooth layer weights W.

    Parameters
      D : (L-1, NX) interface depths
      NZ: vertical size
      t_*: logistic thickness controls; vary along x and are smoothed

    Returns
      W : float32 array, shape (L, NZ, NX), sum over layers ≈ 1 per cell
    """
    Lm1, NX = D.shape
    L = Lm1 + 1
    ...
    # Per‑interface thickness and crispness masks
    T = np.zeros((Lm1, NX), dtype=np.float32)
    crisp_masks: List[np.ndarray] = []
    ...
    for i in range(Lm1):
        ...
        # Build a blended "crispness" mask c(x) using RBF blobs
        ...
        # Blend crisp (thin) and soft (thick) thickness per x
        T[i, :] = c * t_crsp + (1.0 - c) * t_soft

    # Logistic gates A(z; x)
    A = 1.0 / (1.0 + np.exp(-(D[None, ...] - z[:, None, :]) / T[None, ...]))  # (NZ, L-1, NX)

    # Extra sharpening over crisp spans
    for i in range(Lm1):
        c = crisp_masks[i]  # (NX,)
        gamma_boost = np.random.uniform(*gate_gamma_range)
        Gamma = 1.0 + c[None, :] * (gamma_boost - 1.0)  # (1, NX)
        A[:, i, :] = _sharpen_gate(A[:, i, :], Gamma)

    # Convert gates → partition of unity weights
    W = []
...
def layered_background_logR(
    ...
    amp_frac: Tuple[float, float] = (0.03, 0.10),
    wavelen_frac: Tuple[float, float] = (0.25, 0.90),
):
    """Construct a layered background log10ρ with waviness concentrated near the top.

    If `return_weights` is True, also returns the layer weights used to blend
    facies values (useful to suppress heterogeneity near contacts).
    """
...
def smooth_ellipse_mask(NZ, NX, cx, cz, ax, az, theta_rad, edge_cells=8.0):
    """Smooth [0,1] mask from ellipse signed‑distance; edge_cells controls ramp width."""
    X, Z = np.meshgrid(np.arange(NX, dtype=np.float32), np.arange(NZ, dtype=np.float32))
...
def add_heterogeneity(
    logR_base: np.ndarray,
    W: np.ndarray | None = None,
    anomaly_masks: List[np.ndarray] | None = None,
    edge_field: np.ndarray | None = None,
    sigma_cap_dex: float = 0.02,
    rel_cap: float = 0.08,
    corr: Tuple[int, int] = (6, 12),
    depth_decay_cells: float | None = 120,
) -> np.ndarray:
    """Add weak, correlated heterogeneity away from interfaces & anomalies.

    Parameters (important)
      sigma_cap_dex : absolute cap in log10 units for noise amplitude
      rel_cap       : relative cap based on dynamic range of logR_base
      corr          : gaussian correlation (σ_z, σ_x) in cells
      depth_decay_cells : exponential decay length vs depth (cells)
    """
    ...
    # Suppress near layer interfaces using gradients of partition weights
    if W is not None:
    ...
    # Keep anomalies clean (reduce heterogeneity inside)
    if anomaly_masks:
        M = np.clip(np.sum(np.stack(anomaly_masks, axis=0), axis=0), 0.0, 1.0)
        A *= (1.0 - 0.7 * M)

    # Also suppress around a provided transition field (e.g., water table)
    ...
    # Optional exponential depth decay
...
def clamp_global(logR: np.ndarray) -> np.ndarray:
    """Clamp to physically sane range in log10(ρ): [0.2, 1e4] Ω·m."""
...
def homogenize_deep(
    logR: np.ndarray,
    z0: int,
    facies_choices: Sequence[str] | None = None,
    t_cells: float | None = None,
) -> np.ndarray:
    """Blend to a near‑homogeneous deep value below depth `z0` via a logistic ramp."""
    NZ, NX = logR.shape
...
def gen_tracer(NZ: int = 100, NX: int = 200) -> np.ndarray:
    """Shallow conductive tracer patches embedded in layered host."""
    ...
    for _ in range(A):
        ...
        edge_cells = (np.random.uniform(1.0, 3.0) if np.random.rand() < 0.4 else np.random.uniform(8.0, 14.0))
    ...
    logR = add_heterogeneity(
        logR, W=W, anomaly_masks=masks, sigma_cap_dex=0.02, rel_cap=0.08, depth_decay_cells=120
    )
    logR = homogenize_deep(logR, z0=z_shallow_max)
    return clamp_global(logR)


def gen_geology(NZ: int = 100, NX: int = 200) -> np.ndarray:
    """Mostly layered geology with long‑wavelength waviness and gentle heterogeneity."""
    ...
    logR = add_heterogeneity(logR, W=W, sigma_cap_dex=0.015, rel_cap=0.06, depth_decay_cells=120)
    logR = homogenize_deep(logR, z0=int(shallow_here * NZ))
    return clamp_global(logR)


def gen_surface(NZ: int = 100, NX: int = 200) -> np.ndarray:
    """Near‑surface gaussian lobes blended into a two‑layer background."""
    logR, W = layered_background_logR(NZ, NX, L=2, return_weights=True)
    ...
    logR = homogenize_deep(
        logR,
        z0=int(SHALLOW_FRAC * NZ),
        facies_choices=["fresh_sat", "brackish_sat", "moist_sandstone", "moist_limestone"],
    )
    return clamp_global(logR)


def gen_seawater(NZ: int = 100, NX: int = 200, shallow_override: float | None = None) -> np.ndarray:
    """Coastal wedge / seawater intrusion scenario with a deepening conductor."""
    shallow_here = float(shallow_override if shallow_override is not None else np.random.uniform(0.75, 0.90))
    logR, W = layered_background_logR(
        NZ, NX, L=np.random.choice([2, 3], p=[0.6, 0.4]), return_weights=True, shallow_frac=shallow_here
    )

    # Build a coast‑parallel interface depth with tilt + waviness
    coast = np.random.choice(["left", "right"])
    x = np.arange(NX, dtype=np.float32)
    u = x / max(1, NX - 1) if coast == "left" else (NX - 1 - x) / max(1, NX - 1)
    ...
    z_if = np.clip(base_depth + tilt + wav, 0.15 * NZ, min(0.70 * NZ, shallow_here * NZ))

    # Thickness varies along x (crisp vs soft spans) + sharpening
    ...
    logR = add_heterogeneity(
        logR, W=W, edge_field=s_deep, sigma_cap_dex=0.02, rel_cap=0.08, depth_decay_cells=120, corr=(8, 14)
    )
    return clamp_global(logR)


def gen_watertable(NZ: int = 100, NX: int = 200) -> np.ndarray:
    """Undulating water table separating near‑surface and saturated facies."""
    ...
    logR = add_heterogeneity(
        logR, W=W, edge_field=s, sigma_cap_dex=0.02, rel_cap=0.08, depth_decay_cells=120, corr=(6, 11)
    )
    return clamp_global(logR)


def gen_resistive(NZ: int = 100, NX: int = 200) -> np.ndarray:
    """Resistive inclusions and optionally a shallow resistive cap layer."""
    logR, W = layered_background_logR(NZ, NX, L=2, return_weights=True)

    # Optional shallow resistive cap
    cap_applied = False
    s_shallow = None
    ...
    # Resistive ellipses (varied orientation/aspect)
    masks: List[np.ndarray] = []
    A = np.random.choice([1, 2, 3], p=[0.6, 0.3, 0.1])
    z_shallow_max = int(SHALLOW_FRAC * NZ)
    for _ in range(A):
    ...
    logR = add_heterogeneity(
        ...
        W=W,
        anomaly_masks=masks,
        edge_field=(s_shallow if cap_applied else None),
        sigma_cap_dex=0.02,
        rel_cap=0.08,
...
def gen_one(case: str, nz: int, nx: int) -> np.ndarray:
    """Generate one log10ρ field for a given case (shape: NZ×NX)."""
    return GENS[case](NZ=nz, NX=nx).astype(np.float32)


def save_all(out_dir: Path, X_list: List[np.ndarray], y_list: List[int], cases_list: List[str]):
    """Stack and save the whole dataset to a compressed NPZ. Returns arrays."""
...
def main():
    ap = argparse.ArgumentParser(description="Generate synthetic 2‑D log10 resistivity fields and save a labeled dataset.")
    ap.add_argument("--out", type=str, default="./ert_ds", help="Output folder")
    ap.add_argument("--nx", type=int, default=200, help="Grid width (columns)")
    ap.add_argument("--nz", type=int, default=100, help="Grid height (rows; depth)")
    ...
    # Save label map & metadata for quick inspection
