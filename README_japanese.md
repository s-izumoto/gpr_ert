...
- **数値シミュレーション:** ERT測定はオープンソースの物理探査ライブラリ *pyGIMLi* を用いて再現します。
...
本リポジトリでは、**Wenner–alpha 配列**を基準配列として採用しています。これは感度分布が対称で、測定設計の最適化手法（GPR逐次設計）との比較基準として扱いやすいためです。
...
本リポジトリでは pyGIMLi を用いて、**Wenner 配列によるERT順解析および逆解析**を数値的に再現しています。
これにより、導電率マップを入力として、**合成測定データ（真値）**を生成し、GPR逐次設計の入力および評価基準として利用しています。
...
- **pyGIMLi** は、これらの順解析・逆解析を数値的に再現するためのオープンソースライブラリであり、本ワークフローの物理計算基盤を担う。  
