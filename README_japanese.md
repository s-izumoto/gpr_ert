...
このリポジトリは、**合成電気比抵抗トモグラフィ（ERT）** と **ガウス過程回帰（GPR）** に基づく **逐次実験設計** の完全なワークフローを実装しています。  
不均一な比抵抗場の生成から、反演、そしてWenner配列を用いた基準との定量的比較までのすべての段階をカバーします。
...
このモジュール化された構成により、パイプラインの**再現・デバッグ・拡張**が容易になります。
    ...
    │   ├── measurements_warmup.py   # 初期（ウォームアップ）Wenner測定の順解析
    │   ├── measurements_post_warmup.py  # 拡張配列型の順解析
    │   ├── gpr_seq_core.py          # ガウス過程回帰（GPR）のコア実装
    │   ├── sequential_GPR.py        # gpr_seq_coreをラップする逐次設計ドライバ
    │   ├── invert_GPR.py            # GPR選択測定データの一括反演
    │   ├── forward_invert_Wenner.py # Wenner配列基準の順解析＋反演ワークフロー
    ...
    │   ├── measurements_warmup.yml  # Wenner配列のウォームアップ順解析設定
    │   ├── measurements_post_warmup.yml # 複数配列型の順解析設定
    │   ├── sequential_GPR.yml       # 逐次GPR設計の設定（カーネル、獲得関数、ウォームアップ長など）
    │   ├── invert_GPR.yml           # 逐次GPR出力の反演設定
    │   ├── forward_invert_Wenner.yml# Wenner配列基準の順解析＋反演設定
...
それぞれが前段の出力を次の入力として受け取ります。
  ...
  **07**                     `07_invert_GPR.py`                 逐次GPRによる測定データを反演し、地下比抵抗分布を再構築。

  **08**                     `08_forward_invert_Wenner.py`      Wenner配列基準の順解析＋反演を行い、参照データを作成。

  **09**                     `09_evaluate_GPR_vs_Wenner.py`     GPRベースとWennerベースの反演結果を、誤差・空間的類似度などで定量評価。
...
4.  **ウォームアップ順解析:** 標準配列構成を用いて初期ERT応答を収集。\
5.  **候補拡張:** 多数の配列構成に対する応答をシミュレーション。\
6.  **逐次GPR:**
    ウォームアップ後のGPRモデルに獲得関数を適用し、最適な次測定点を逐次選択。\
7.  **反演:** 逐次取得データに基づいて地下比抵抗を再構築。\
8.  **基準反演:** Wenner基準で同様の反演を実施。\
...
-   **フィールドごとのログ出力**（CSV,
    NPZ）でカーネルハイパーパラメータ、獲得統計、反演品質を記録
...
4.  **ウォームアップ測定シミュレーション（Wenner配列）**\
    pyGIMLiを用いてWenner-α配列で順解析を実施し、GPR初期化用データを収集。
...
5.  **拡張測定シミュレーション（複数配列）**\
    Schlumberger, Dipole-Dipole, Gradientなど追加配列で順解析を実施。
...
7.  **逐次GPR結果の反演**\
    選択された測定データを用いて反演を実施し、地下比抵抗モデルを再構築。
